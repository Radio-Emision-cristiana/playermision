...
function toggleTheme() {
    // Solo tema oscuro disponible - no se puede cambiar a claro
    ...
    // Siempre mantener tema oscuro activo
    ...
    // Ocultar icono del sol, mostrar icono de la luna
    if (sunIcon) sunIcon.style.display = 'none';
    if (moonIcon) moonIcon.style.display = 'block';
    
    console.log('🌙 Tema oscuro - único disponible');
}
...
function loadTheme() {
    ...
    // Guardar preferencia como tema oscuro
    localStorage.setItem('theme', 'dark');
    
    console.log('🌙 Tema oscuro cargado como único tema disponible');
}
...
// ========================================
// EFECTO DE NIEVE CAYENDO DINÁMICO
// ========================================

function initSnowEffect() {
    console.log('❄ Inicializando efecto de nieve...');
    
    const snowContainer = document.getElementById('snow-container');
    if (!snowContainer) {
        console.log('⚠️ Contenedor de nieve no encontrado');
        return;
    }

    // Configuración de la nieve
    const snowConfig = {
        maxFlakes: 50,           // Máximo número de copos
        minSize: 8,              // Tamaño mínimo
        maxSize: 16,             // Tamaño máximo
        speed: 1,                // Velocidad base
        wind: 2                  // Fuerza del viento
    };
    ...
    function createSnowflake() {
        ...
        flake.className = 'snowflake dynamic-flake';
        ...
        flake.style.fontSize = (snowConfig.minSize + Math.random() * (snowConfig.maxSize - snowConfig.minSize)) + 'px';
        
        // Color aleatorio con transparencia
        const opacity = 0.4 + Math.random() * 0.6;
        flake.style.opacity = opacity;
        
        // Añadir movimiento horizontal aleatorio
        const horizontalMovement = (Math.random() - 0.5) * snowConfig.wind * 20;
        flake.style.setProperty('--horizontal-move', horizontalMovement + 'px');
    ...
    // Crear copos de forma continua
    let flakeCount = 20;
    const createFlakeInterval = setInterval(() => {
        if (flakeCount < snowConfig.maxFlakes) {
            createSnowflake();
            flakeCount++;
        } else {
            clearInterval(createFlakeInterval);
        }
    }, 500);

    // Efecto de viento ocasional
    setInterval(() => {
        const flakes = document.querySelectorAll('.dynamic-flake');
        if (Math.random() > 0.7) { // 30% probabilidad
            flakes.forEach(flake => {
                const windChange = (Math.random() - 0.5) * 20;
                flake.style.setProperty('--horizontal-move', windChange + 'px');
            });
        }
    }, 3000);

    console.log('✅ Efecto de nieve activado con', snowConfig.maxFlakes, 'copos máximo');
}
...
// Función para abrir donaciones de PayPal
function openPayPalDonation() {
    console.log('💰 Abriendo donaciones de PayPal...');
    
    // URL de donaciones de PayPal (reemplazar con tu URL real de PayPal.me)
    ...
    if (typeof Swal !== 'undefined') {
        Swal.fire({
            icon: 'info',
            title: 'Donaciones PayPal',
            text: 'Te redirigiremos a PayPal para realizar tu donación.',
            timer: 2000,
    ...
    setTimeout(() => {
        if (isMobileDevice) {
            // En dispositivos móviles, intentar abrir en nueva ventana
        ...
        console.log('💰 Redirigiendo a PayPal:', paypalUrl);
    }, 500);
}
